...
const TagInput = ({ value = [], onChange, placeholder }) => {
  ...
  return (
    <div className="p-2 border rounded flex flex-wrap gap-2 items-center bg-white min-h-[42px]">
      {value.map((tag, index) => (
        <span key={index} className="pill bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full flex items-center gap-1">
          {tag}
      ...
      <input
        ...
        className="outline-none flex-grow min-w-[150px] p-1 text-sm bg-transparent"
      />
    </div>
  );
};
